...
- (id)valueForObject:(id)object {
     ...
     * @see https://github.com/CoderMJLee/MJExtension/issues/545
     */
 ...
 * 对应着字典中的key
 */
- (void)setOriginKey:(id)originKey forClass:(Class)cls {
    if ([originKey isKindOfClass:[NSString class]]) {
        ...
        if (propertyKeys.count) {
            [self setPropertyKeys:@[propertyKeys] forClass:cls];
        }
        ...
        if (keyses.count) {
            [self setPropertyKeys:keyses forClass:cls];
        }
    }
}
 ...
 * 对应着字典中的多级key
 */
- (void)setPropertyKeys:(NSArray *)propertyKeys forClass:(Class)cls {
...
- (NSArray *)propertyKeysForClass:(Class)cls {
    NSString *key = NSStringFromClass(cls);
    if (!key) return nil;
 ...
 * 模型数组中的模型类型
 */
- (void)setObjectClassInArray:(Class)objectClass forClass:(Class)cls {
...
- (Class)objectClassInArrayForClass:(Class)cls {
    NSString *key = NSStringFromClass(cls);
    if (!key) return nil;
