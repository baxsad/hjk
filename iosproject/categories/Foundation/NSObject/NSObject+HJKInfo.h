...
/// @param block 用于遍历的 block
+ (void)hjk_enumrateIvarsOfClass:(Class)aClass includingInherited:(BOOL)includingInherited usingBlock:(void (^)(Ivar ivar, NSString *ivarDescription))block;
...
/// @param block 用于遍历的 block，如果要获取 property 的信息。
+ (void)hjk_enumratePropertiesOfClass:(Class)aClass includingInherited:(BOOL)includingInherited usingBlock:(void (^)(objc_property_t property, NSString *propertyName))block;

/// 使用 block 遍历当前实例的所有方法，不包含 superclasses 里定义的 method
/// @param block 用于遍历的 block
- (void)hjk_enumrateInstanceMethodsUsingBlock:(void (^)(Method method, SEL selector))block;

/// 使用 block 遍历指定的某个类的实例方法
/// @param aClass 指定的 class
/// @param includingInherited 是否要包含由继承链带过来的 method
/// @param block 用于遍历的 block
+ (void)hjk_enumrateInstanceMethodsOfClass:(Class)aClass includingInherited:(BOOL)includingInherited usingBlock:(void (^)(Method method, SEL selector))block;

/// 遍历某个 protocol 里的所有方法
/// @param protocol 要遍历的 protocol，例如 \@protocol(xxx)
/// @param block 遍历过程中调用的 block
+ (void)hjk_enumerateProtocolMethods:(Protocol *)protocol usingBlock:(void (^)(SEL selector))block;
@end
