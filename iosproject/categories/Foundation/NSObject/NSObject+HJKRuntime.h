...
- (BOOL)hjk_hasOverrideMethod:(SEL)selector ofSuperclass:(Class)superclass;

/// 判断指定的类是否有重写某个父类的指定方法
/// @param selector 要判断的方法
/// @param aClass 要比较的类
/// @param superclass 要比较的父类，必须是当前类的某个 superclass
+ (BOOL)hjk_hasOverrideMethod:(SEL)selector forClass:(Class)aClass ofSuperclass:(Class)superclass;
...
- (nullable id)hjk_performSelectorToSuperclass:(SEL)aSelector;
...
/// @return 消息执行后的结果
/// @link http://stackoverflow.com/questions/14635024/using-objc-msgsendsuper-to-invoke-a-class-method @/link
- (nullable id)hjk_performSelectorToSuperclass:(SEL)aSelector withObject:(nullable id)object;
...
- (void)hjk_performSelector:(SEL)selector withPrimitiveReturnValue:(nullable void *)returnValue;
...
- (void)hjk_performSelector:(SEL)selector withPrimitiveReturnValue:(nullable void *)returnValue arguments:(nullable void *)firstArgument, ...;

/// 调用一个带参数的 selector，参数类型支持对象和非对象，也没有数量限制。返回值为对象或者 void。
/// @param selector 要被调用的方法名
/// @param firstArgument 参数列表，请传参数的指针地址，支持多个参数
/// @return 方法的返回值，如果该方法返回类型为 void，则会返回 nil，如果返回类型为对象，则返回该对象。
- (nullable id)hjk_performSelector:(SEL)selector withArguments:(nullable void *)firstArgument, ...;
@end
