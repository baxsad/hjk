...
typedef UIViewController<HJKNavigationBarTransitionDelegate> HJKTransitionViewController;
typedef UIViewController<HJKNavigationBarAppearanceDelegate> HJKBarAppearanceController;
...
- (void)addTransitionNavigationBarIfNeeded {
    if (!self.view.hjk_visible || !self.navigationController.navigationBar) {
        return;
    }
    
    UINavigationBar *originBar = self.navigationController.navigationBar;
    HJKTransitionNavigationBar *customBar = [[HJKTransitionNavigationBar alloc] init];
...
- (void)resizeTransitionNavigationBarFrame {
    if (!self.view.hjk_visible) {
        return;
    }
    
    UIView *backgroundView = self.navigationController.navigationBar.hjk_backgroundView;
...
- (void)renderNavigationStyleInViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    if ([viewController conformsToProtocol:@protocol(HJKNavigationBarTransitionDelegate)] &&
        [viewController conformsToProtocol:@protocol(HJKNavigationBarAppearanceDelegate)]) {
        HJKTransitionViewController *transitionViewController = (HJKTransitionViewController *)viewController;
        ...
        HJKBarAppearanceController *barAppearanceController = (HJKBarAppearanceController *)viewController;
        ...
        if (tintColor) {
            if (@available(iOS 11, *)) {
                if (self.navigationController.hjk_isPopping) {
                    UILabel *backButtonLabel = viewController.navigationController.navigationBar.hjk_backButtonLabel;
                    if (backButtonLabel) {
                        backButtonLabel.hjk_specifiedTextColor = backButtonLabel.textColor;
                        [viewController hjk_animateAlongsideTransition:nil completion:^(id<UIViewControllerTransitionCoordinatorContext>  _Nonnull context) {
                            backButtonLabel.hjk_specifiedTextColor = nil;
                        }];
                    }
                }
            }
            
            [viewController hjk_animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext>  _Nonnull context) {
...
- (BOOL)shouldCustomTransitionAutomaticallyForOperation:(UINavigationControllerOperation)operation firstViewController:(UIViewController *)viewController1 secondViewController:(UIViewController *)viewController2 {
    if (![viewController1 conformsToProtocol:@protocol(HJKNavigationBarTransitionDelegate)] ||
        ![viewController2 conformsToProtocol:@protocol(HJKNavigationBarTransitionDelegate)] ||
        ![viewController1 conformsToProtocol:@protocol(HJKNavigationBarAppearanceDelegate)] ||
        ![viewController2 conformsToProtocol:@protocol(HJKNavigationBarAppearanceDelegate)]) {
        return NO;
    }
    ...
    if ([viewController1.navigationController.delegate respondsToSelector:@selector(navigationController:animationControllerForOperation:fromViewController:toViewController:)]) {
        BOOL a = [(HJKTransitionViewController *)viewController1 respondsToSelector:@selector(shouldCustomizeNavigationBarTransitionIfUsingCustomTransitionForOperation:fromViewController:toViewController:)] ? [(HJKTransitionViewController *)viewController1 shouldCustomizeNavigationBarTransitionIfUsingCustomTransitionForOperation:operation fromViewController:viewController1 toViewController:viewController2] : NO;
        BOOL b = [(HJKTransitionViewController *)viewController2 respondsToSelector:@selector(shouldCustomizeNavigationBarTransitionIfUsingCustomTransitionForOperation:fromViewController:toViewController:)] ? [(HJKTransitionViewController *)viewController2 shouldCustomizeNavigationBarTransitionIfUsingCustomTransitionForOperation:operation fromViewController:viewController1 toViewController:viewController2] : NO;
    ...
    if ([viewController1 respondsToSelector:@selector(customNavigationBarTransitionKey)]) {
        transitionKey1 = [(HJKTransitionViewController *)viewController1 customNavigationBarTransitionKey];
    }
    
    if ([viewController2 respondsToSelector:@selector(customNavigationBarTransitionKey)]) {
        transitionKey2 = [(HJKTransitionViewController *)viewController2 customNavigationBarTransitionKey];
    }
    ...
    if ([viewController1 respondsToSelector:@selector(navigationBarBackgroundImage)]) {
        backgroundImage1 = [(HJKBarAppearanceController *)viewController1 navigationBarBackgroundImage];
    } else {
    ...
    if ([viewController2 respondsToSelector:@selector(navigationBarBackgroundImage)]) {
        backgroundImage2 = [(HJKBarAppearanceController *)viewController2 navigationBarBackgroundImage];
    } else {
    ...
    if (backgroundImage1 || backgroundImage2) {
        ...
        if (![backgroundImage1.hjk_averageColor isEqual:backgroundImage2.hjk_averageColor]) {
            /**
             * 目前只能判断图片颜色是否相等了
             */
            return YES;
    ...
    if (!backgroundImage1 && !backgroundImage2) {
        ...
        if ([viewController1 respondsToSelector:@selector(navigationBarBarTintColor)]) {
            barTintColor1 = [(HJKBarAppearanceController *)viewController1 navigationBarBarTintColor];
        } else {
        ...
        if ([viewController2 respondsToSelector:@selector(navigationBarBarTintColor)]) {
            barTintColor2 = [(HJKBarAppearanceController *)viewController2 navigationBarBarTintColor];
        } else {
        ...
        if ([viewController1 respondsToSelector:@selector(navigationBarStyle)]) {
            barStyle1 = [(HJKBarAppearanceController *)viewController1 navigationBarStyle];
        } else {
            barStyle1 = [UINavigationBar appearance].barStyle;
        }
        
        if ([viewController2 respondsToSelector:@selector(navigationBarStyle)]) {
            barStyle2 = [(HJKBarAppearanceController *)viewController2 navigationBarStyle];
        } else {
    ...
    if ([viewController1 respondsToSelector:@selector(navigationBarShadowImage)]) {
        shadowImage1 = [(HJKBarAppearanceController *)viewController1 navigationBarShadowImage];
    } else {
    ...
    if ([viewController2 respondsToSelector:@selector(navigationBarShadowImage)]) {
        shadowImage2 = [(HJKBarAppearanceController *)viewController2 navigationBarShadowImage];
    } else {
    ...
    if (shadowImage1 || shadowImage2) {
        ...
        if (![shadowImage1.hjk_averageColor isEqual:shadowImage2.hjk_averageColor]) {
            return YES;
        }
    }
...
- (UIColor *)containerViewBackgroundColor {
    ...
    if ([self conformsToProtocol:@protocol(HJKNavigationBarAppearanceDelegate)]) {
        HJKTransitionViewController *transitionViewController = (HJKTransitionViewController *)self;
...
- (void)setTransitionNavigationBar:(HJKTransitionNavigationBar *)value {
    objc_setAssociatedObject(self, @selector(transitionNavigationBar), value, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (HJKTransitionNavigationBar *)transitionNavigationBar {
    return (HJKTransitionNavigationBar *)objc_getAssociatedObject(self, _cmd);
}

- (void)setPrefersNavigationBarBackgroundViewHidden:(BOOL)value {
    if (value) {
        self.navigationController.navigationBar.hjk_backgroundView.layer.mask = [CALayer layer];
    } else {
        self.navigationController.navigationBar.hjk_backgroundView.layer.mask = nil;
    }
