...
@implementation HJKTraceroute

- (HJKTraceroute *)initWithMaxTTL:(int)ttl timeout:(int)timeout maxAttempts:(int)attempts port:(int)port {
    self = [super init];
    if (self) {
        maxTTL = ttl;
        udpPort = port;
...
- (Boolean)doTraceRoute:(NSString *)host {
    NSArray *serverDNSs = [HJKHostTools remoteDNSServersWithHost:host];
    ...
    while (ttl <= maxTTL) {
        ...
        for (int try = 0; try < maxAttempts; try ++) {
            startTime = [HJKTimeTools getMicroSeconds];
            ...
            if (FD_ISSET(recv_sock, &readfds) > 0) {
                ...
                if ((res = recvfrom(recv_sock, buf, 100, 0, (struct sockaddr *)&fromAddr, &n)) < 0) {
                    ...
                    icmp = true;
                    delta = [HJKTimeTools computeDurationSince:startTime];
