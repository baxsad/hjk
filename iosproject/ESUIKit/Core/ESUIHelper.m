...
#import "ESUIHelper.h"
#import "ESUIDefines.h"
#import "ESUICategories.h"
#import <math.h>
#import <sys/utsname.h>

@implementation ESUIHelper

@end

@implementation ESUIHelper (Device)

+ (NSString *)deviceModel {
    if (ESUIHelper.isSimulator) {
...
+ (NSString *)deviceName {
    ...
    dispatch_once(&onceToken, ^{
        NSString *model = ESUIHelper.deviceModel;
        if (!model) {
            name = @"Unknown Device";
            return;
        }
        ...
        if (ESUIHelper.isSimulator) {
            name = name.esui_stringByAppending(@" Simulator");
        }
    });
    return name;
}
...
+ (NSInteger)numberiOSVersion {
    NSString *OSVersion = [[UIDevice currentDevice] systemVersion];
    NSArray *OSVersionArr = OSVersion.esui_componentsSeparatedBy(@".");
...
+ (BOOL)isIPhone {
    if (isIPhone < 0) {
        ...
        isIPhone = string.esui_rangeOf(@"iPhone").location != NSNotFound ? 1 : 0;
    }
    return isIPhone > 0;
}
        ...
        if (isNotchedScreen < 0) {
            if (@available(iOS 12.0, *)) {
                /*
                 ...
                 2. 与触发 [QMUIHelper isNotchedScreen] 方法时的进程有关。
                ...
                [[UIScreen mainScreen] esui_performSelector:peripheryInsetsSelector withPrimitiveReturnValue:&peripheryInsets];
                ...
                isNotchedScreen = ESUIHelper.is58InchScreen ? 1 : 0;
            }
        }
    } else {
        isNotchedScreen = 0;
...
+ (BOOL)isRegularScreen {
    if (ESUIHelper.isIPad) {
        return YES;
    }
    
    if (ESUIHelper.isZoomedMode) {
        return NO;
    }
    
    if (ESUIHelper.is65InchScreen ||
        ESUIHelper.is61InchScreen ||
        ESUIHelper.is55InchScreen) {
        return YES;
    }
...
+ (BOOL)is67InchScreen {
    if (is67InchScreen < 0) {
        if (DeviceWidth == ESUIHelper.screenSizeFor67Inch.width &&
            DeviceHeight == ESUIHelper.screenSizeFor67Inch.height) {
            is67InchScreen = 1;
        } else {
            is67InchScreen = 0;
        }
...
+ (BOOL)is65InchScreen {
    if (is65InchScreen < 0) {
        if (DeviceWidth == ESUIHelper.screenSizeFor65Inch.width &&
            DeviceHeight == ESUIHelper.screenSizeFor65Inch.height) {
            if (ESUIHelper.deviceModel.esui_isEqualTo(@"iPhone11,4") ||
                ESUIHelper.deviceModel.esui_isEqualTo(@"iPhone11,6") ||
                ESUIHelper.deviceModel.esui_isEqualTo(@"iPhone12,5")) {
                is65InchScreen = 1;
            } else {
                is67InchScreen = 0;
            }
...
+ (BOOL)is61InchScreenAndiPhone12 {
    if (is61InchScreenAndiPhone12 < 0) {
        if (DeviceWidth == ESUIHelper.screenSizeFor61InchAndiPhone12.width &&
            DeviceHeight == ESUIHelper.screenSizeFor61InchAndiPhone12.height) {
            if (ESUIHelper.deviceModel.esui_isEqualTo(@"iPhone13,2") ||
                ESUIHelper.deviceModel.esui_isEqualTo(@"iPhone13,3")) {
                is61InchScreenAndiPhone12 = 1;
            } else {
                is61InchScreenAndiPhone12 = 0;
            }
        } else {
            is61InchScreenAndiPhone12 = 0;
        }
...
+ (BOOL)is61InchScreen {
    if (is61InchScreen < 0) {
        if (DeviceWidth == ESUIHelper.screenSizeFor61Inch.width &&
            DeviceHeight == ESUIHelper.screenSizeFor61Inch.height) {
            if (ESUIHelper.deviceModel.esui_isEqualTo(@"iPhone11,8") ||
                ESUIHelper.deviceModel.esui_isEqualTo(@"iPhone12,1")) {
                is61InchScreen = 1;
            } else {
                is61InchScreen = 0;
            }
...
+ (BOOL)is58InchScreen {
    if (is58InchScreen < 0) {
        if (DeviceWidth == ESUIHelper.screenSizeFor58Inch.width &&
            DeviceHeight == ESUIHelper.screenSizeFor58Inch.height) {
            is58InchScreen = 1;
        } else {
            is58InchScreen = 0;
        }
...
+ (BOOL)is55InchScreen {
    if (is55InchScreen < 0) {
        if (DeviceWidth == ESUIHelper.screenSizeFor55Inch.width &&
            DeviceHeight == ESUIHelper.screenSizeFor55Inch.height) {
            is55InchScreen = 1;
        } else {
            is55InchScreen = 0;
        }
...
+ (BOOL)is54InchScreen {
    if (is54InchScreen < 0) {
        if (DeviceWidth == ESUIHelper.screenSizeFor54Inch.width &&
            DeviceHeight == ESUIHelper.screenSizeFor54Inch.height) {
            is54InchScreen = 1;
        } else {
            is54InchScreen = 0;
        }
...
+ (BOOL)is47InchScreen {
    if (is47InchScreen < 0) {
        if (DeviceWidth == ESUIHelper.screenSizeFor47Inch.width &&
            DeviceHeight == ESUIHelper.screenSizeFor47Inch.height) {
            is47InchScreen = 1;
        } else {
            is47InchScreen = 0;
        }
...
+ (BOOL)is40InchScreen {
    if (is40InchScreen < 0) {
        if (DeviceWidth == ESUIHelper.screenSizeFor40Inch.width &&
            DeviceHeight == ESUIHelper.screenSizeFor40Inch.height) {
            is40InchScreen = 1;
        } else {
            is40InchScreen = 0;
        }
...
+ (BOOL)is35InchScreen {
    if (is35InchScreen < 0) {
        if (DeviceWidth == ESUIHelper.screenSizeFor35Inch.width &&
            DeviceHeight == ESUIHelper.screenSizeFor35Inch.height) {
            is35InchScreen = 1;
        } else {
            is35InchScreen = 0;
        }
...
+ (UIEdgeInsets)safeAreaInsetsForDeviceWithNotch {
    ...
    NSString *deviceKey = ESUIHelper.deviceModel;
    if (!dict[deviceKey]) {
        /// 默认按 iPhone 11 Pro Max
        deviceKey = @"iPhone12,5";
    }
...
+ (CGFloat)statusBarHeightConstant {
    if (UIApplication.sharedApplication.statusBarHidden) {
        if (ESUIHelper.isIPad) {
            ...
            if (ESUIHelper.isNotchedScreen) {
                if (ESUIHelper.isLandscape) {
                    return 0;
                } else {
                    if (ESUIHelper.deviceModel.esui_isEqualTo(@"iPhone12,1")) {
                        return 48;
                    } else if (ESUIHelper.is61InchScreenAndiPhone12 || ESUIHelper.is67InchScreen) {
                        return 47;
                    } else {
                        return 44;
                    }
...
+ (CGFloat)navigationContentHeight {
    return ESUIHelper.statusBarHeight + ESUIHelper.navigationBarHeight;
}

+ (CGFloat)navigationContentHeightConstant {
    return ESUIHelper.statusBarHeightConstant + ESUIHelper.navigationBarHeight;
}
...
+ (CGFloat)tabBarHeight {
    if (ESUIHelper.isIPad) {
        ...
        if (ESUIHelper.isLandscape) {
            if (ESUIHelper.isRegularScreen) {
                return 49 + ESUIHelper.safeAreaInsetsForDeviceWithNotch.bottom;
            } else {
                return 32 + ESUIHelper.safeAreaInsetsForDeviceWithNotch.bottom;
            }
        } else {
            return 49 + ESUIHelper.safeAreaInsetsForDeviceWithNotch.bottom;
        }
...
@implementation ESUIHelper (ViewController)

+ (nullable UIViewController *)visibleViewController {
    UIViewController *rootViewController = UIApplication.sharedApplication.delegate.window.rootViewController;
    UIViewController *visibleViewController = [rootViewController esui_visibleViewControllerIfExist];
