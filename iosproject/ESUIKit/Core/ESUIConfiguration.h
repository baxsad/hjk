...
@interface ESUIConfiguration : NSObject
...
@interface UIViewController (ESUIConfiguration)
- (NSArray <UIViewController *>*)esui_existingViewControllersOfClass:(Class)class;
@end

#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 130000

UIKIT_EXTERN API_AVAILABLE(ios(13.0), tvos(13.0)) @interface UITabBarAppearance (ESUIConfiguration)

/**
 * 同时设置 stackedLayoutAppearance、inlineLayoutAppearance、compactInlineLayoutAppearance 三个状态下的 itemAppearance
 */
- (void)esui_applyItemAppearanceWithBlock:(void (^)(UITabBarItemAppearance *itemAppearance))block;
@end

#endif

@interface UITabBarItem (ESUIConfiguration)
- (void)esui_updateTintColorForiOS12AndEarlier:(nullable UIColor *)tintColor;
@end
