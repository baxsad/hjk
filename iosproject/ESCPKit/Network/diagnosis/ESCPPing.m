...
@interface ESCPPing () <SimplePingDelegate>
...
- (void)sendPing {
    ...
    if (_sendCount > MAXCOUNT_PING) {
        ...
        assert(self.ping != nil);
        _sendCount++;
        _startTime = [ESCPTimeTools getMicroSeconds];
        if (_isLargePing) {
            NSString *testStr = @"";
            for (int i=0; i<408; i++) {
...
#pragma unused(packet)
    ...
    NSString *successLog = [NSString
        stringWithFormat:@"%lu bytes from %@ icmp_seq=#%u type=%@ time=%ldms",
                         ...
                         sequenceNumber,
                         icmpReplyType,
                         [ESCPTimeTools computeDurationSince:_startTime] / 1000];
