...
#define strongifySelf __strong typeof(weakSelf) self = weakSelf;
...
- (void)_SR_commonInit {
    ...
    assert([scheme isEqualToString:@"ws"] ||
...
- (void)open {
    ...
    if (_urlRequest.timeoutInterval > 0) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, _urlRequest.timeoutInterval * NSEC_PER_SEC), dispatch_get_main_queue(), ^(void){
            if (self.readyState == SR_CONNECTING)
                [self _failWithError:[NSError errorWithDomain:@"com.squareup.SocketRocket"
                                                         code:504
...
- (void)_HTTPHeadersDidFinish {
    ...
    if (responseCode >= 400) {
        ...
        [self _failWithError:[NSError errorWithDomain:SRWebSocketErrorDomain
                                                 code:2132
                                             userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:@"received bad response code from server %ld", (long)responseCode], SRHTTPResponseErrorKey:@(responseCode)}]];
        return;
    }
...
#pragma clang diagnostic pop
    ...
    for (NSString *cookieKey in cookies) {
        ...
        if ([cookieKey length] && [cookieValue length]) {
            CFHTTPMessageSetHeaderFieldValue(request,
                                             (__bridge CFStringRef)cookieKey,
                                             (__bridge CFStringRef)cookieValue);
        }
    }
...
#pragma clang diagnostic pop
        ...
        CFHTTPMessageSetHeaderFieldValue(request,
                                         CFSTR("Authorization"),
                                         (__bridge CFStringRef)_basicAuthorizationString);
    }
...
- (void)_handleFrameWithData:(NSData *)frameData opCode:(NSInteger)opcode {
    ...
    BOOL isControlFrame = (opcode == SROpCodePing ||
                           opcode == SROpCodePong ||
...
- (void)_readFrameContinue {
    ...
    [self _addConsumerWithDataLength:2 callback:^(SRWebSocket *self, NSData *data) {
        ...
        BOOL isControlFrame = (receivedOpcode == SROpCodePing ||
...
- (void)_pumpWriting {
    ...
    if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
        ...
        if (bytesWritten == -1) {
            [self _failWithError:[NSError errorWithDomain:SRWebSocketErrorDomain
                                                     code:2145
                                                 userInfo:[NSDictionary dictionaryWithObject:@"Error writing to stream" forKey:NSLocalizedDescriptionKey]]];
             return;
        }
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data {
    ...
    NSAssert([data isKindOfClass:[NSData class]] ||
             [data isKindOfClass:[NSString class]], @"NSString or NSData");
